...
class ActionRegressionDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        rgb_image = data['rgb']
        new_order = (2, 0, 1)
        image_tranposed = torch.permute(rgb_image,new_order)

        target_normalized = np.array([data['center_point'][0]/128, data['center_point'][1]/128, data['angle']/180])
        
        data_torch = {
            'input': image_tranposed/255,
...
class ActionRegressionModel(nn.Module):
    ...
    def predict_grasp(self, rgb_obs: np.ndarray
        ...
        # ===============================================================================
        image = torch.tensor(rgb_obs, dtype=torch.float32)
        order = (2,0,1)
        image = torch.permute(image, order) / 255
        ...
        coord, angle = recover_action(pred[0])
        # ===============================================================================
        # visualization
        image_input = np.array(rgb_obs) * 255
        image_input = np.transpose(image_input, order)
        vis_img = self.visualize(image_input, pred[0])  
