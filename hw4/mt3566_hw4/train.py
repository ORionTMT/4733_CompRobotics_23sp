...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        left_coord, right_coord = get_finger_points(data_torch["center_point"], data_torch["angle"])
        ...
        image = data_torch["rgb"].numpy()
        image_aug, kps_aug = self.aug_pipeline(image=image, keypoints=kps)

        left_coord_new= np.array([kps_aug.keypoints[0].x, kps_aug.keypoints[0].y])
        right_coord_new = np.array([kps_aug.keypoints[1].x, kps_aug.keypoints[1].y])

        center_point, angle = get_center_angle(left_coord_new, right_coord_new)
